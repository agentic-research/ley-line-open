...
Every base-op response except `query` is typed against `daemon.capnp` via [`rs/ll-open/cli-lib/src/daemon/wire.rs`](rs/ll-open/cli-lib/src/daemon/wire.rs). `BaseRequest` is a serde tagged enum (`#[serde(tag = "op", rename_all = "snake_case")]`) so unknown ops, malformed args, and missing required fields surface as structured errors rather than silent miss-and-coerce. The JSON encoding is the carrier today; the typed contract is the schema. `query` is the deliberate escape hatch — its `rows` shape is column-keyed maps rather than the schema's positional `List(Text)` (tracked as a follow-up; the wire-shape change can't be reconciled additively). See ADR-0014's *Out of scope (future ADRs) → Live RPC* section for the framing.
