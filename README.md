...
The current release is `v0.11.2`. It publishes platform binaries, FFI
staticlibs, and the Apache-2.0 Go schema module at
`clients/go/leyline-schema/v0.11.2`. See
...
`localhost/leyline:0.11.2` (equivalently `ley-line-open:0.11.2`). It uses
krust/cargo-zigbuild for the static binary
and `cgr.dev/chainguard/static:latest` as the runtime base. The image is not
automatically pushed to GHCR; `ghcr.io/agentic-research/ley-line-open:0.11.2`
