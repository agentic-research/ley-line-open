...
**Resolution (coarse v1, 2026-07-07)** — PR #140 landed the watcher-driven emit with a coarse `scope: "all-known"` payload: every currently-known region ID from the persisted `CellComplex` fires on every reparse. Correct (no stale data) but wasteful (any file change fans out to every region).

**Resolution (fine-grained v2, 2026-07-08)** — bead `ley-line-open-e40566`. Path (a) via the daemon-owned mapping route: `ComplexBuildPass` produces a `region_id → token label` map alongside its `CellComplex` (the token→id assignment already exists inside `build_complex`; the pass now installs the inverse into `SheafState.region_labels`), and `emit_watcher_sheaf_invalidate` computes `changed_files → region_ids` by matching labels of shape `<path>` or `<path>:sym:<NAME>`. Payload's new `scope: "changed-only"` value carries the fine-grained diff; `scope: "all-known"` remains as the backward-compat fallback for mache-pushed topologies (which never carry labels) and fresh-startup daemons. No consumer-side registration protocol required — path (b) is unnecessary because the daemon already owns the observation table that produces the labelled tokens. New wire fields: none. Existing consumers (mache's `SheafSubscriber`) parse the same `region_ids` + `count` fields and immediately get precise invalidation.
