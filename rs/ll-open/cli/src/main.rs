...
#[derive(Parser)]
#[command(name = "ll-open", about = "Pre-bake source code into a .db for mache")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Parse {
            ...
            output,
            lang,
        } => cmd_parse(&source, &output, lang.as_deref()),
    }
}

fn cmd_parse(source: &Path, output: &Path, lang_filter: Option<&str>) -> Result<()> {
    if !source.is_dir() {
...
fn walk_children(
    ...
    let mut kind_counts = std::collections::HashMap::<&str, usize>::new();
    ...
    let mut kind_indices = std::collections::HashMap::<&str, usize>::new();
