...
"#,
    ...
    let request = ExecutionRequest {
        run_id: "listener-probe".into(),
        replay_key: "listener-probe-replay".into(),
        rootfs: DigestRef {
    ...
    assert!(
        ...
        "SECURITY: the host reached a guest listener on port {GUEST_PORT} that \
         no manifest declared. confinement/v1 §4 says an omitted `port` block \
         means the workload MUST NOT bind a listener, and libkrun's documented \
         NULL port_map default is to expose every listening guest port. Call \
         krun_set_port_map with an EMPTY array so the default is closed."
    );
}
