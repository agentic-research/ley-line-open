...
fn produce_go_fixture_for_mache() {
    ...
    assert!(
        node_count > 10,
        "should have nodes for functions/types/identifiers"
    );
    ...
    assert!(
        fn_count >= 2,
        "should have 2+ function_declaration AST entries, got {fn_count}"
    );
    ...
    assert!(
        id_count >= 2,
        "should have identifier AST entries, got {id_count}"
    );
